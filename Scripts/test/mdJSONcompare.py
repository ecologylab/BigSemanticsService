...
#don't print equal ones or mm_name

for x in range(0, len(clientList2)):
	curKey = clientList2[x].keys()[0]
	for k in clientList2[x][curKey]:
		if k == "mm_name":
			clientList[x][curKey].pop(k, None)
		if not isinstance(clientList2[x][curKey][k], dict):
			for y in range(0, len(clientList2[x][curKey][k])):
				for k2 in clientList2[x][curKey][k][y]: 
					if k2 == "mm_name":
						clientList[x][curKey][k][y].pop(k2, None)
		if isinstance(clientList2[x][curKey][k], dict):
			for y in clientList2[x][curKey][k]:
				if y == "mm_name":
					clientList[x][curKey][k].pop(y, None)
"""
for x in range(0, len(servList2)):
	curKey = servList2[x].keys()[0]
	if curKey in clientList2[x]:
		for k in servList2[x][curKey]:
			if k == 'location':
				continue
			if k in clientList[x][curKey] and k in servList[x][curKey]:
				print clientList[x][curKey][k]
				print servList[x][curKey][k]
				if clientList[x][curKey][k] == servList[x][curKey][k]:
					print "deleting"
					servList[x][curKey].pop(k, None)
					clientList[x][curKey].pop(k, None)
"""
	...
	for metadataClient in clientList:
		...
		if curKey in metadataClient.keys():
			if metadataServ[curKey]['location'] == metadataClient[curKey]['location']:
				print "SERVER"
				print json.dumps(metadataServ, sort_keys=True, indent=4, separators=(',', ': '))
				print "CLIENT"
				print json.dumps(metadataClient, sort_keys=True, indent=4, separators=(',', ': '))
				i+=1
	if i > 1:
		break
...
def nestedCheck(serv, client):		
	...
	if isinstance(serv, dict) and isinstance(client, dict):
		for k2 in serv:
				if k2 in client and serv[k2] == client[k2]:
					nestedMatchCount += 1
		if len(serv.keys()) == nestedMatchCount:
			return True
	else:
		#print str(len(serv.keys()))
		#print str(nestedMatchCount)
		#print serv
		#print client
		return False
			...
			for k in metadataServ[curKey]:
				if k in metadataClient[curKey]:
					if metadataServ[curKey][k] == metadataClient[curKey][k]:
						matchCount += 1
					#check nested elem
					elif isinstance(metadataServ[curKey][k], dict):
						if nestedCheck(metadataServ[curKey][k], metadataClient[curKey][k]):
							matchCount += 1
						else:
							print "NESTED MISMATCH"
					elif isinstance(metadataServ[curKey][k], list):
						nestedMatchCount = 0
						for i in range(0, len(metadataServ[curKey][k])):
							if i < len(metadataClient[curKey][k]): 
								if nestedCheck(metadataServ[curKey][k][i], metadataClient[curKey][k][i]):
									nestedMatchCount += 1
						if len(metadataServ[curKey][k]) == nestedMatchCount:
							matchCount += 1
						else:
							#print str(len(metadataServ[curKey][k]))
							#print str(nestedMatchCount)
							print "LIST NESTED MISMATCH"
