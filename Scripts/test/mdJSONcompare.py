				...
				for k in serv[i]:	
					if k == "download_status":
						...
						for j in range(0, len(client)):
							if k in client[j]:
								if serv[i][k] == client[j][k] or nestedCheck(serv[i][k], client[j][k]):
									#print "match because content"
									nestedMatchCount += 1
				if len(serv[i].keys()) == nestedMatchCount:
					listItemMatchCount += 1
				elif i < len(client) and 'mm_name' in client[i] and client[i]['mm_name'] == 'instructable':
					print "Server keys " + str(len(serv[i].keys()))
					print "Matches " + str(nestedMatchCount)
					#prettyPrint(serv)
					#prettyPrint(client)
